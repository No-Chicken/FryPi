...
static float normalizetempValues[32*24];
...
static void drawPicture(void) {
  ...
  uint8_t data[2];
  LCD_Address_Set(12+20 , 15 , 12+20+32*8-1, 15+24*8-1);//设置显示范围
  //pix_horizontal
	for (y=0; y<24; y++) {
    //pix_vertical
		for (x=0; x<32; x++) {
...
void mlx90640_display_process(void)
{
  ...
  //user ai run
  normalizeArray();
  if(user_ai_run(normalizetempValues, outputs))
  {
    printf("\r\n run erro \r\n");
  }
	uint8_t temp = findMaxIndex(outputs, sizeof(outputs) / sizeof(outputs[0]));
	printf("\r\npredict gesture:%d\r\n", temp);
	uint8_t strbuf[5];
	sprintf(strbuf, "%d", temp);
	LCD_ShowString(160, 210, strbuf, WHITE, BLACK, 32, 0);
